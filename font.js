...
Font.prototype.validate = function (target, zero, mark, font) {
  ...
  // validation paragraph, but leave the actual font stylesheet.
  if (width > 0) {
  ...
  else { setTimeout(function () { font.validate(target, zero, mark, font); }, 50); }
};
...
Font.prototype.ondownloaded = function () {
  ...
  // Create a validation stylesheet for the requested font:
  ...
  // If there is getComputedStyle, we do proper load completion verification.
    ...
    var local = this;
    var delayedValidate = function() { local.validate(para, zerowidth, realfont, local); };
    setTimeout(delayedValidate, 50);
  }
};

/**
 * This gets called when font.src is set, effected
 * by the very last line in this file.
 */
